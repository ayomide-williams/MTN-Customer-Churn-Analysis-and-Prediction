...
class EDA:
    ...
    def feature_importance_analysis(self, features: List[str], save_path: Optional[str] = None):
        ...
        for idx, feature in enumerate(features):
            ...
            if is_categorical_dtype(feature_series) or is_string_dtype(feature_series) or feature_series.dtype == object:
            ...
            elif is_numeric_dtype(feature_series):
                # For numerical features
                plot_df = self.df[[feature, self.target_column]].copy()
                plot_df[feature] = pd.to_numeric(plot_df[feature], errors='coerce')
                plot_df = plot_df.dropna(subset=[feature, self.target_column])
                if plot_df.empty:
                    ax.text(0.5, 0.5, 'No valid numeric data', ha='center', va='center', fontsize=11)
                    ax.set_title(f'{feature} vs Churn', fontsize=12, fontweight='bold')
                ...
                ax.text(0.5, 0.5, 'Unsupported feature type', ha='center', va='center', fontsize=11)
                ax.set_title(f'{feature} vs Churn', fontsize=12, fontweight='bold')
                ax.set_xlabel('Churn Status', fontsize=10)
                ax.set_ylabel(feature, fontsize=10)
